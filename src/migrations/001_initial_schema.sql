...
CREATE TABLE IF NOT EXISTS tenants (
  ...
  CONSTRAINT tenants_email_check CHECK (email ~* '^[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Z]{2,}$'),
...
CREATE TABLE IF NOT EXISTS billing_events (
  ...
  CONSTRAINT events_type_check CHECK (
    event_type IN ('api_call', 'storage_used', 'compute_time', 'custom')
  ),
...
CREATE POLICY events_tenant_isolation ON billing_events
  USING (tenant_id = current_setting('app.tenant_id')::uuid)
  WITH CHECK (tenant_id = current_setting('app.tenant_id')::uuid);
